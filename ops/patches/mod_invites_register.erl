...
check_captcha(true, #register{xdata = X}, #iq{lang = Lang} = IQ) ->
    ...
    case ejabberd_captcha:process_reply(XdataC) of
        ok ->
            case process_xdata_submit(X) of
                ...
                _ ->
                    ...
                    {error, make_stripped_error(IQ, #register{}, xmpp:err_bad_request(Txt, Lang))}
            end;
        {error, malformed} ->
        ...
        _ ->
            ...
            {error, make_stripped_error(IQ, #register{}, xmpp:err_not_allowed(ErrText, Lang))}
    end;
check_captcha(false, #register{xdata = #xdata{} = X}, #iq{lang = Lang} = IQ) ->
    case process_xdata_submit(X) of
        {ok, _} = Result ->
            Result;
        _ ->
